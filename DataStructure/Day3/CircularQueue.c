/*
 ============================================================================
 Name        : CircularQueue_Of_Integers.c
 Author      : 
 Version     :
...
{
	//1. keep on adding element till size-1 but nosingle ele is deleted
	//2. keep on adding element till size-1 no. of eles we del we add same no. of eles
...
void join_element(QUEUE  *queue,ELEMENT ele)
{
	...
	queue->eles[queue->rear]=ele;//actually adds ele in Q
...
{
	//1. empty initially rear==-1 //2. after performing operations on
...
void leave_element(QUEUE *queue)
{
   ...
   //decide which will be next front element
	    if(queue->front == queue->rear) //there was single ele which got deleted
	      {
	          queue->rear=-1;	//reset Q
	          queue->front=-1;
